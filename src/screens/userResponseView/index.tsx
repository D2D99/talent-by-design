...
const FilterSection = ({
  ...
  return (
    <div className="border-b border-gray-100 last:border-0 pb-6">
      ...
      <div
        className={`overflow-hidden transition-all duration-300 ${isOpen ? "max-h-[1000px] opacity-100 mt-4" : "max-h-0 opacity-0"}`}
      >
        {children}
      </div>
    </div>
...
const UserResponseView = () => {
  ...
  const filteredResponses = useMemo(() => {
    return responses.filter((r) => {
      if (filterDomains.length > 0 && !filterDomains.includes(r.domain))
        return false;
      if (
        filterSubdomains.length > 0 &&
        !filterSubdomains.includes(r.subdomain)
      )
        return false;
      if (filterTypes.length > 0 && !filterTypes.includes(r.questionType))
        return false;
      return true;
    });
        ...
        <div className="space-y-6">
          {loading ? (
            <div className="py-20 text-center flex flex-col items-center">
              ...
              <p className="text-gray-400 text-sm mt-4 font-semibold uppercase tracking-widest">
                Loading Records...
              </p>
            </div>
                  ...
                  >
                    <h2 className="mb-0" id={`heading-${safeId}`}>
                      ...
                      >
                        ...
                        <span
                          className={`ms-auto h-6 w-6 shrink-0 transition-transform duration-200 ease-in-out flex items-center justify-center rounded-full bg-gradient-to-t ${isOpen ? "rotate-[-180deg] from-[#1a3652] to-[#448bd2] text-white" : "rotate-0 !text-[var(--primary-color)] from-[var(--light-primary-color)] to-[var(--light-primary-color)]"}`}
                        >
                    ...
                    <div
                      id={`collapse-${safeId}`}
                      className={`!visible ${isOpen ? "" : "hidden"}`}
                      ...
                      <div className="px-4 text-sm sm:px-6 pb-6 pt-2">
                        <div className="space-y-5">
                          {questionsInGroup.map((resp, qIdx) => (
                            <div
                              key={resp._id}
                              className="flex justify-between items-start group bg-white border-b last:border-0 pb-5 sm:flex-row flex-col gap-y-2.5"
                            >
                              ...
                              <div className="flex sm:gap-3 gap-1.5 lg:gap-2 transition-opacity whitespace-nowrap pt-1 lg:pt-0 self-start shrink-0 justify-end sm:w-fit w-full">
                                <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest bg-gray-100 px-2.5 py-0.5 rounded-full border border-gray-100">
