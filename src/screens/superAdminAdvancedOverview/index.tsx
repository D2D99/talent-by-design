...
const SuperAdminAdvancedOverview = () => {
  ...
  const recentActivities = [
    ...
    {
      id: 2,
      org: "Future Systems Inc",
      action: "New Invitations Sent",
      time: "5 hours ago",
      status: "In Progress",
    },
    {
  ...
  return (
    <div className="min-h-screen p-4 sm:p-6 space-y-8 animate-in fade-in duration-700">
      {/* Header Section */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-white/40 backdrop-blur-xl border border-white/20 p-6 rounded-2xl shadow-xl">
        <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, index) => (
          ...
          >
            ...
            <div
              className="absolute -right-4 -bottom-4 opacity-5 group-hover:opacity-10 transition-opacity duration-500"
              style={{ color: stat.color }}
            >
          ...
          <div className="bg-white rounded-2xl border border-slate-100  shadow-xl overflow-hidden">
              ...
              <div className="flex bg-white p-1 rounded-lg border border-slate-200">
                {["all", "growth", "stable"].map((tab) => (
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-md text-sm font-bold transition-all duration-300 capitalize ${
                      activeTab === tab
            ...
            <div className="p-8 h-[400px] flex flex-col items-center justify-center text-center relative">
              ...
              <p className="mt-6 text-slate-500 max-w-md italic">
                Advanced cross-organization correlation analysis will be
                visualized here once more assessment data is compiled.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100  shadow-xl p-6">
            <h3 className="text-xl font-bold text-slate-800 mb-6 flex items-center gap-2">
              <Icon
                ...
                className="text-[#FF512F] animate-pulse"
              />
              Intelligence Pulse
            </h3>
            <div className="space-y-6">
              {recentActivities.map((activity) => (
                ...
                >
                  <div className="relative">
                    <div
                      className={`w-3 h-3 rounded-full mt-1.5 ${activity.status === "Success" ? "bg-emerald-500" : "bg-amber-500"}`}
                    ></div>
          ...
          <div className="bg-gradient-to-br from-[#1a3652] to-[#448bd2] rounded-2xl shadow-xl p-6 text-white relative overflow-hidden">
            <div className="relative z-10">
              ...
              <p className="text-blue-100 text-sm mb-4">
                New Predictive Analysis module is now available for Super
                Admins.
              </p>
