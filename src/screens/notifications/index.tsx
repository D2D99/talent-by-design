...
const NotificationHistory = () => {
  ...
  return (
    <div className="bg-[var(--app-surface)] border border-[var(--app-border-color)] shadow-[4px_4px_4px_0px_rgba(68,140,210,0.1)] dark:shadow-[0_8px_30px_rgba(0,0,0,0.3)] sm:p-6 p-4 rounded-[12px] mt-6 min-h-[calc(100vh-162px)] transition-all duration-300">
      ...
      {/* Tabs */}
      <div className="flex gap-4 mb-6 overflow-x-auto no-scrollbar sticky top-[80px] z-10 bg-[var(--app-surface)]">
        {(["All", "Unread", "Read"] as const).map((tab) => (
          ...
          >
            ...
            {tab === "All" && notifications.length > 0 && (
              <span className="ml-2 bg-white text-[var(--primary-color)] text-xs py-0.5 px-[7px] rounded-full backdrop-blur-sm">
      ...
      <div className="space-y-3">
        {filteredNotifications.length > 0 ? (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex sm:items-center justify-between gap-4 mb-1">
                  <h4
                    className={`text-base leading-snug truncate ${!notif.isRead ? "font-bold text-[#1a3652]" : "font-semibold text-gray-800"}`}
                  >
                    {notif.title}
                  </h4>
                ...
                <p
                  className={`text-sm leading-relaxed ${!notif.isRead ? "text-gray-700 font-medium" : "text-gray-500"}`}
                >
                  {notif.message}
                </p>
              </div>
          ...
          <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
            <div className="w-20 h-20 bg-primary-500/5 rounded-full flex items-center justify-center mb-4 shadow-sm border border-gray-100">
              <Icon
                icon="solar:bell-off-outline"
                className="text-gray-300 w-10 h-10"
              />
            </div>
            <h5 className="text-gray-900 font-bold mb-1 text-lg">
              No notifications found
            </h5>
            <p className="text-sm text-gray-500 max-w-lg leading-relaxed">
              You don't have any notifications in this category yet. We'll alert
