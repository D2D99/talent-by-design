...
const PersonDeepDive = () => {
        ...
        const fetchData = async () => {
            ...
            try {
                const [reportRes, avgRes] = await Promise.all([
                    ...
                    api.get(`dashboard/manager-team-avg?userId=${userId}&includeSelf=true`),
                ]);
    ...
    const roleAverages = (() => {
        ...
        const getRoles = (roleData: any, label: string, count: number) => {
            const scores = Object.values(roleData || {}).map((d: any) => d.avgScore || 0);
            const score = scores.length > 0 ? Math.round(scores.reduce((a, b) => a + b, 0) / scores.length) : 0;
            return { label: `${label} (${count})`, value: score, color: getMetricColor(score) };
        };
            ...
            <div className="mb-8">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                    <div>
                        ...
                        <div className="flex items-center gap-4 text-sm text-gray-500">
                            <span className="flex items-center gap-1">
                                <Icon icon="solar:buildings-2-bold" className="text-[#448CD2]" />
                                {orgName}
                            </span>
                            <span className="flex items-center gap-1">
                    ...
                    <div className="flex gap-2">
                        <button
                            onClick={() => navigate(`/dashboard/reports/${userData?.role?.toLowerCase() === 'admin' ? 'org-head' : userData?.role?.toLowerCase()}?userId=${userId}&orgName=${encodeURIComponent(orgName || '')}`)}
                    ...
                    <div className="mt-8 space-y-4">
                        {roleAverages.map((r, i) => (
                            <div key={i} className="flex justify-between items-center p-3 bg-slate-50 rounded-xl">
                                <span className="text-sm font-bold text-gray-700">{r.label.split(' (')[0]}</span>
                    ...
                    <div className="sm:flex-row justify-between items-start mb-10 gap-4">
                        ...
                        <div className="flex mt-4 justify-center items-center gap-4 px-4 py-2 rounded-xl ">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-xs font-semibold text-[#64748B]">Attention</span>
                            </div>
                        </div>
                    </div>
