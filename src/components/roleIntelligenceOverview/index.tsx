...
type IntelligenceScope = "admin" | "manager" | "leader";

interface RoleIntelligenceOverviewProps {
  scope: IntelligenceScope;
}

const PARTICIPATION_DEPARTMENTS = [
  "HR/People & Culture",
  "Finance & Accounting",
  "Operations",
  "IT",
...
const RoleIntelligenceOverview = ({
  scope,
}: RoleIntelligenceOverviewProps) => {
  ...
  const fetchIntelligence = async (showLoader = true) => {
    try {
      ...
      const res = await api.get(`assessment/${scope}/intelligence?range=${timeRange}`);
  ...
  const people = intelData?.people || [];
  const normalizeRole = (value: string) => value.trim().toLowerCase();
  const normalizedAssessmentStatus = (value: string) =>
    value.trim().toLowerCase();
  const isAssessmentAssigned = (person: any) => {
    const assessmentStatus = normalizedAssessmentStatus(
      String(person.assessmentStatus || ""),
    );
    const inviteStatus = normalizedAssessmentStatus(String(person.status || ""));
    ...
    return [
      ...
      "not started",
      "pending",
    ].includes(assessmentStatus) || ["invited", "registered"].includes(inviteStatus);
  };
  const derivedRoleBreakdown = {
    manager: people.filter((person: any) => normalizeRole(String(person.role || "")) === "manager").length,
    leader: people.filter((person: any) => normalizeRole(String(person.role || "")) === "leader").length,
    employee: people.filter((person: any) => normalizeRole(String(person.role || "")) === "employee").length,
  };
  const totalMembers = stats?.totalMembers || 0;
  const completedAssessments = stats?.completedAssessments || 0;
  const completionRate = stats?.completionRate || 0;
  const inProgressAssessments = stats?.inProgressAssessments || 0;
  const notStartedAssessments = stats?.notStartedAssessments || 0;
  ...
  const displayStats = [
    ...
    },
    {
      ...
      value: (stats?.activeInvites || 0).toString(),
    ...
    },
    {
      ...
      value: Math.max(totalMembers - completedAssessments, 0).toString(),
  ...
  const roleSegments =
    scope === "admin"
      ? [
          {
            label: "Managers",
            value: roleBreakdown?.manager ?? derivedRoleBreakdown.manager,
            color: "#10B981",
          },
          {
            label: "Leaders",
            value: roleBreakdown?.leader ?? derivedRoleBreakdown.leader,
            color: "#6366F1",
          },
          {
            label: "Employees",
            value: roleBreakdown?.employee ?? derivedRoleBreakdown.employee,
            color: "#F59E0B",
          },
        ]
      : scope === "leader"
  ...
  const roleLabels = roleSegments.map((segment) => segment.label);
  const roleData = roleSegments.map((segment) => segment.value);
  const roleColors = roleSegments.map((segment) => segment.color);
  const primaryAction =
    scope === "admin"
      ? {
      ...
      : scope === "leader"
        ? {
            label: "Review Leader Report",
            to: "/dashboard/reports/senior-leader",
          }
        : {
            label: "Review Manager Report",
            to: "/dashboard/reports/manager",
          };
  const topPerformers = [...people]
    .filter((person: any) => isAssessmentAssigned(person))
    .sort((a: any, b: any) => {
      const scoreA = typeof a.lastScore === "number" ? a.lastScore : -1;
      const scoreB = typeof b.lastScore === "number" ? b.lastScore : -1;
      return scoreB - scoreA;
    })
    .slice(0, 4);
  const normalizeDepartment = (value: string) =>
    value.trim().toLowerCase().replace(/\s+/g, " ");
  const participantDepartments = PARTICIPATION_DEPARTMENTS.map((department) => {
    const count = people.filter((person: any) => {
      const personDepartment = String(person.department || "");

      return (
        isAssessmentAssigned(person) &&
        normalizeDepartment(personDepartment) === normalizeDepartment(department)
      );
    }).length;
  ...
  return (
    <div className="bg-white border border-[#448CD2] border-opacity-20 shadow-[4px_4px_4px_0px_#448CD21A] sm:p-6 p-4 rounded-[12px] mt-6 min-h-[calc(100vh-162px)] space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        <div
          className={`${scope === "admin" ? "xl:col-span-5" : "xl:col-span-8"} bg-[var(--app-surface)] rounded-[20px] border border-[var(--app-border-color)] p-7 flex flex-col shadow-sm relative overflow-hidden`}
        >
            ...
            <div>
              ...
              <div className="w-full h-3 bg-[var(--app-surface-soft)] rounded-full border border-[var(--app-border-color)]/30 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${totalMembers > 0 ? (inProgressAssessments / totalMembers) * 100 : 0}%`,
                  }}
                ></div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="w-full h-3 bg-[var(--app-surface-soft)] rounded-full border border-[var(--app-border-color)]/30 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${totalMembers > 0 ? (notStartedAssessments / totalMembers) * 100 : 0}%`,
                  }}
                ></div>
              </div>
            </div>
      ...
      <div className="bg-[var(--app-surface)] rounded-[24px] border border-[var(--app-border-color)] overflow-hidden shadow-sm">
        <div className="p-8 border-b border-[var(--app-border-color)] flex items-center justify-between gap-4">
          <div>
            <h3 className="text-xl font-black text-[var(--app-heading-color)] tracking-tight">
              Top Performance
            </h3>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">
              Live participation and leading scores
            </p>
          </div>
          <div className="w-11 h-11 rounded-full bg-[var(--app-surface-soft)] border border-[var(--app-border-color)] flex items-center justify-center text-[var(--primary-color)]">
            <Icon icon="solar:ranking-bold-duotone" width="22" />
          </div>
        </div>

        {topPerformers.length > 0 ? (
          <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-4 p-6">
            {topPerformers.map((person: any, index: number) => (
              <div
                key={`${person.email}-${index}`}
                className="rounded-[18px] border border-[var(--app-border-color)] bg-[var(--app-surface-soft)] p-5"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-11 h-11 rounded-full bg-[var(--primary-color)]/10 flex items-center justify-center text-[var(--primary-color)] font-black text-sm border border-[var(--primary-color)]/20 shrink-0">
                      {person.name?.charAt(0) || "U"}
                    </div>
                    <div className="min-w-0">
                      <p className="text-sm font-bold text-[var(--app-heading-color)] truncate">
                        {person.name}
                      </p>
                      <p className="text-[10px] font-black uppercase tracking-wider text-slate-400 truncate mt-1">
                        {person.role}
                      </p>
                    </div>
                  </div>
                ...
                <div className="mt-5 flex items-end justify-between gap-3">
                  <div>
                    ...
                    <p className="text-3xl font-black text-[var(--app-heading-color)] tracking-tight">
                      {typeof person.lastScore === "number"
                        ? `${person.lastScore}%`
                        : "--"}
                    </p>
                  </div>
                  {person.classification ? (
                    <span
                      className={`text-[9px] font-black px-2 py-1 rounded-full border uppercase ${
                    ...
                    <span className="text-[9px] font-black px-2 py-1 rounded-full border uppercase bg-blue-50 text-blue-700 border-blue-200">
                      {person.assessmentStatus || person.status || "Assigned"}
                    </span>
                  )}
                </div>
              </div>
          ...
          <div className="px-8 py-10 text-center text-sm text-slate-400 italic">
            No assigned assessment participants available yet.
          </div>
        )}
      </div>
      ...
      <div className="bg-[var(--app-surface)] rounded-[24px] border border-[var(--app-border-color)] overflow-hidden shadow-sm">
        <div className="p-8 border-b border-[var(--app-border-color)] flex items-center justify-between gap-4">
          ...
          <div className="w-11 h-11 rounded-full bg-[var(--app-surface-soft)] border border-[var(--app-border-color)] flex items-center justify-center text-[var(--primary-color)]">
            <Icon icon="solar:buildings-3-bold-duotone" width="22" />
          </div>
        </div>

        <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4 p-6">
          {participantDepartments.map((item) => (
            <div
              key={item.department}
              className="rounded-[18px] border border-[var(--app-border-color)] bg-[var(--app-surface-soft)] px-5 py-4 flex items-center justify-between gap-4"
            >
              <div>
                <p className="text-sm font-bold text-[var(--app-heading-color)]">
                  {item.department}
                </p>
              ...
              <div className="min-w-12 h-12 rounded-full bg-white border border-[var(--app-border-color)] flex items-center justify-center text-xl font-black text-[var(--primary-color)] shadow-sm">
                {item.count}
              </div>
            </div>
          ))}
...
export default RoleIntelligenceOverview;
